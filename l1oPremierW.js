...
async function scrapeL1OPremierWStandings() {
  const leagueName = 'League1 Ontario Premier (W)';
  const url = 'https://www.league1ontario.com/competition/league-1-womens-premier/';
  ...
  try {
    ...
    return {
      ...
      source: "live_scrape",
      divisions: [{
        name: "2025 Women Premier Division",
        teams
      }]
    };
  } catch (err) {
    console.error(`❌ League1 Premier scrape failed: ${err.message}`);
    return { league: leagueName, error: err.message, divisions: [] };
  }
}

module.exports = scrapeL1OPremierWStandings;
