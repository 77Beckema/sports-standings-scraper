...
async function scrapeL2OSouthwestStandings() {
  const leagueName = 'League2 Ontario Southwest';
  const url = 'https://league1ontario.com/competition/league-2-mens/southwest/';
  ...
  try {
    ...
    return {
      ...
      source: "live_scrape",
      divisions: [{
        name: "2025 Men League 2 Southwest Division",
        teams
      }]
    };
  } catch (err) {
    console.error(`❌ League2 Southwest scrape failed: ${err.message}`);
    return { league: leagueName, error: err.message, divisions: [] };
  }
}

module.exports = scrapeL2OSouthwestStandings;