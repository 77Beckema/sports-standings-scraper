...
async function scrapeL1OChampionshipStandings() {
  const leagueName = 'League1 Ontario Championship';
  const url = 'https://league1ontario.com/competition/league1-championship-mens/';
  ...
  try {
    ...
    return {
      ...
      source: "live_scrape",
      divisions: [{
        name: "2025 Men Championship Division",
        teams
      }]
    };
  } catch (err) {
    console.error(`❌ League1 Championship scrape failed: ${err.message}`);
    return { league: leagueName, error: err.message, divisions: [] };
  }
}

module.exports = scrapeL1OChampionshipStandings;