...
async function scrapeL2OSouthwestWStandings() {
  const leagueName = 'League2 Ontario Southwest (W)';
  const url = 'https://league1ontario.com/competition/league-2-womens/southwest/';

  try {
    console.log("Navigating to League2 Southwest Ontario standings page...");
    ...
    return {
      ...
      source: "live_scrape",
      divisions: [{
        name: "2025 Women League 2 Southwest Division",
        teams
      }]
    };
  } catch (err) {
    console.error(`❌ League2 Southwest W scrape failed: ${err.message}`);
    return { league: leagueName, error: err.message, divisions: [] };
  }
}

module.exports = scrapeL2OSouthwestWStandings;