...
async function scrapeL2ONortheastStandings() {
  const leagueName = 'League2 Ontario Northeast Division';
  const url = 'https://league1ontario.com/competition/league-2-mens/northeast/';
  ...
  try {
    ...
    return {
      ...
      source: "live_scrape",
      divisions: [{
        name: "2025 Men League 2 Northeast Division",
        teams
      }]
    };
  } catch (err) {
    console.error(`❌ League2 Northeastscrape failed: ${err.message}`);
    return { league: leagueName, error: err.message, divisions: [] };
  }
}

module.exports = scrapeL2ONortheastStandings;