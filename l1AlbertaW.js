...
async function scrapeL1AlbertaWStandings() {
  const leagueName = 'League1 Alberta (W)';
  const url = 'https://league1alberta.com/competition/league1-womens/';

  try {
    console.log("Navigating to League1 Alberta standings page...");
    ...
    return {
      ...
      source: "live_scrape",
      divisions: [{
        name: "2025 Women ALberta Division",
        teams
      }]
    };
  } catch (err) {
    console.error('❌ League1 Alberta (w) scrape failed: ${err.message}');
    return { league: leagueName, error: err.message, divisions: [] };
  }
}

module.exports = scrapeL1AlbertaWStandings;