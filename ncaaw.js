...
async function scrapeNCAAWomensStandings() {
  const leagueName = 'NCAA Division I Men';
  const url = 'https://www.ncaa.com/rankings/soccer-women/d1/ncaa-womens-soccer-rpi';
...
lines.forEach(line => {
  ...
  if (parts.length === 8 && /^\d{1,3}$/.test(parts[0])) {
    ...
    console.log("✅ NCAA W scrape returned", teams.length, "teams");
    return {
      league: leagueName,
      source: 'text_parse',
      divisions: [{
    ...
    console.error(`❌ NCAA W standings scrape failed: ${err.message}`);
...
module.exports = scrapeNCAAWomensStandings;
