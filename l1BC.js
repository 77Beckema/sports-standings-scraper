...
async function scrapeL1BCStandings() {
  const leagueName = 'League1 British Columbia';
  const url = 'https://league1bc.ca/competition/league1-mens/';
  ...
  try {
    ...
    return {
      ...
      source: "live_scrape",
      divisions: [{
        name: "2025 Men BC Division",
        teams
      }]
    };
  } catch (err) {
    console.error('❌ League1 BC scrape failed: ${err.message}');
    return { league: leagueName, error: err.message, divisions: [] };
  }
}

module.exports = scrapeL1BCStandings;