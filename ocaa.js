...
async function scrapeOCAAMensStandings() {
  const leagueName = "OCAA Men's Soccer";
  ...
  try {
    ...
    console.log('Navigating to OCAA standings page...');
    ...
    const output = {
      league: leagueName,
      divisions: [{
        name: "2023–24 Regular Season",
        teams
      }]
    };
    fs.writeFileSync('standings_data/standings_ocaa.json', JSON.stringify(output, null, 2));
    console.log(`✅ OCAA standings scraped: ${teams.length} teams saved.`);
    return output; // This prevents undefined in the updater

  } catch (err) {
    console.error(`❌ OCAA standings scrape failed: ${err.message}`);
  }
}
...
module.exports = scrapeOCAAMensStandings;
